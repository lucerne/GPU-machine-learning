                            ...
                            float seed_thresh_min, float seed_thresh_max)
{
  ...
  if (0 <= k && k < width*height) {
    ...
    if (seed_thresh_min <= img[k] && img[k] <= seed_thresh_max) {
                            ...
                            float grow_thresh_min, float grow_thresh_max)
{
  ...
  if (0 <= gid && gid < front_in_length) {
    ...
    // If this pixel is not in the grow region and it could be
    if (grow[k] == 0 &&
        grow_thresh_min <= img[k] && img[k] <= grow_thresh_max) {
      // Flag this pixel as in the grow region
      grow[k] = 1;

      // Add neighbours to the front.  TODO: make coalesced
      if (i > 0)
        front_out[gid + 0*front_in_length] = k - width;
      if (i < height - 1)
        front_out[gid + 1*front_in_length] = k + width;
      if (j > 0)
        front_out[gid + 2*front_in_length] = k - 1;
      if (j < width - 1)
        front_out[gid + 3*front_in_length] = k + 1;
    }
  }
}
"""
...
if __name__ == '__main__':
  ...
  out_file_name = "Harvard_GrowRegion_GPU_B.png"
  ...
  seed_kernel = nvcc.SourceModule(seed2_source).get_function("seed2_image")
  grow_kernel = nvcc.SourceModule(grow2_source).get_function("grow2_image")
  start_time = cu.Event()
  end_time = cu.Event()
